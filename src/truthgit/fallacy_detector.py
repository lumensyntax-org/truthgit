...
def _check_affirming_consequent(text: str) -> bool:
    ...
    affirms_then = bool(re.search(r'(therefore|so|thus|hence)\s+.*(did|was|is|happened)', lower, re.IGNORECASE))
...
def _check_denying_antecedent(text: str) -> bool:
    ...
    lower = text.lower()
    has_conditional = bool(re.search(r'if\s+.+,?\s*(then)?\s+.+', lower, re.IGNORECASE))
    denies_if = bool(re.search(r"(didn't|doesn't|not|never)\s+.*(therefore|so|thus)", lower, re.IGNORECASE))
...
def _check_false_dilemma(text: str) -> bool:
    ...
    lower = text.lower()
    return bool(re.search(r'either\s+.+\s+or\s+', lower, re.IGNORECASE)) or \
           bool(re.search(r'only\s+(two|2)\s+(options|choices|ways)', lower, re.IGNORECASE))
...
def _check_ad_hominem(text: str) -> bool:
    ...
    pattern2 = re.search(
        r"(idiot|liar|fool|stupid|hypocrite).*(so|therefore|hence)",
        lower, re.IGNORECASE
    )
    # Pattern 3: "don't trust/listen to X because..."
    pattern3 = re.search(
        r"don't\s+(listen|trust|believe)\s+(to|in)?\s*\w+\s+(because|since)",
        lower, re.IGNORECASE
    )
...
def _check_straw_man(text: str) -> bool:
    ...
    lower = text.lower()
    return bool(re.search(r"so\s+(you're|you are)\s+saying", lower, re.IGNORECASE)) or \
           bool(re.search(r"what\s+(he|she|they)\s+(really|actually)\s+mean", lower, re.IGNORECASE))


def _check_appeal_to_authority(text: str) -> bool:
    """Using authority as evidence instead of actual evidence."""
    lower = text.lower()
    return bool(re.search(r'(expert|scientist|doctor|study|research)\s+(says?|said|shows?|proved?)', lower, re.IGNORECASE))


def _check_slippery_slope(text: str) -> bool:
    """Assuming a chain of events without justification."""
    lower = text.lower()
    pattern1 = re.search(r'(if|once)\s+we\s+.+,?\s+(then|next|soon|eventually)', lower, re.IGNORECASE)
...
def _check_hasty_generalization(text: str) -> bool:
    ...
    has_limited_sample = bool(re.search(r'(met|saw|know|heard of)\s+(one|a|two|few|some)', lower, re.IGNORECASE))
...
def _check_appeal_to_emotion(text: str) -> bool:
    ...
    lower = text.lower()
    return bool(re.search(r'think\s+of\s+the\s+(children|victims|families)', lower, re.IGNORECASE)) or \
           bool(re.search(r'how\s+would\s+you\s+feel', lower, re.IGNORECASE))
...
def detect_fallacies(argument: str, context: Optional[str] = None) -> FallacyResult:
    """
    ...
    if len(fallacies) == 0:
        ...
        recommendation = f"Multiple fallacies detected ({len(fallacies)}). The argument requires restructuring."

    return FallacyResult(
        valid=len(fallacies) == 0,
        fallacies=fallacies,
